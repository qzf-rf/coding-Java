...
- **思路**：`fast` 指针每次前进 **2 步**，`slow` 指针每次前进 **1 步**
...
> #### **题目 142：环形链表 II **
> 给定一个链表，返回链表开始入环的第一个节点，无环则返回 null
...
- **思路**：**BFS 算法**
  - 利用 visited 记录访问过的密码，避免回头路，直到找到目标密码为止，找不到则返回 -1
  - 四个拨轮为 `String` 类型，拨动单个拨轮为 `char` 类型

- **注意事项**：
  - **遍历与否**在加入相邻节点时检查
  - 别只顾框架，忘了**步数**和**返回值**
  - 代码没有错误，却执行出错，LeetCode 后台错误？

> 时间复杂度 O(N * A<sup>N</sup>)，空间复杂度 O(A<sup>N</sup>)<br>
> A 为数字个数，N 为密码位数
